...
class InterestAreaTriggerSerializer(serializers.Serializer):
    trigger_dict = serializers.DictField()


class PersonInterestAreaSerializer(serializers.Serializer):
    interest_area_dict = serializers.DictField()
    observation_id = serializers.IntegerField()


class FullInterestAreaSerializer(serializers.Serializer):
    interest_area_dict = serializers.DictField(child=InterestAreaTriggerSerializer())


class InterestAreaCreateSerializer(serializers.Serializer):
    interest_area_ids = serializers.ListField(child=serializers.IntegerField())

    def create(self, validated_data):
        user = self.context.get("request").user
        person = get_object_or_404(Person, user=user)
        concept_ids = validated_data.get("interest_area_ids", [])

        person_interests = Observation.objects.get(
            person=person,
        ...
        for concept_id in concept_ids:
            try:
                interest_area = Observation.objects.filter(observation_concept_id=concept_id).first()
                interest_name = interest_area.observation_source_value

                if interest_name in interest_data:
                    continue
                ...
                if interest_area:
                    ...
                    interest_data[interest_name] = [trigger.observation_source_value for trigger in triggers]

            except Concept.DoesNotExist:
                logger.warning(f"Concept with ID {concept_id} not found")
                continue

        person_interests.value_as_string = json.dumps(interest_data)
        person_interests.observation_date = timezone.now()
        person_interests.save()

        return interest_area


class CustomInterestAreaCreateSerializer(serializers.Serializer):
    interest_area_name = serializers.CharField(required=True)
    trigger_names = serializers.ListField(child=serializers.CharField(), required=False, default=[])

    def create(self, validated_data):
        user = self.context.get("request").user
        person = get_object_or_404(Person, user=user)
        interest_name = validated_data.get("interest_area_name")
        trigger_names = validated_data.get("trigger_names", [])
        ...
        interest_data = json.loads(person_interests.value_as_string)
        interest_data[interest_name] = [trigger for trigger in trigger_names]
...
class InterestAreaUpdateSerializer(serializers.Serializer):
    ...
    def update(self, instance, validated_data):
        user = self.context.get("request").user
        person = get_object_or_404(Person, user=user)
        interest_area_name = validated_data.get("interest_area_name")
        trigger_names = validated_data.get("trigger_names", [])
        ...
        if interest_area_name not in json.loads(interest_area.value_as_string):
            raise serializers.ValidationError(f"Interest area '{interest_area_name}' does not exist.")

        triggers = []
        if "trigger_names" in validated_data:
            for trigger_data in trigger_names:
                triggers.append(trigger_data)

        interest_data = json.loads(interest_area.value_as_string)
        interest_data[interest_area_name] = triggers
        interest_area.value_as_string = json.dumps(interest_data)
        interest_area.save()

        return {"interest_area": interest_data[interest_area_name]}


# class PersonInterestAreaRetrieveSerializer(serializers.Serializer):
#     interest_areas = serializers.ListField(
#         child=InterestAreaSerializer(),
#         help_text="List of interest areas associated with the person",
#     )
#     triggers = serializers.ListField(
#         child=InterestAreaTriggerSerializer(),
#         help_text="List of triggers associated with the interest areas",
#     )

#     def to_representation(self, instance):
#         """
#         Transform the JSON storage format into a structured representation
#         with actual Observation objects for interest areas and triggers.
#         """
#         logger.info(f"Serializing PersonInterestAreaRetrieve: {instance}")

#         # Parse the JSON from value_as_string
#         try:
#             if isinstance(instance, dict):
#                 # If we're given a dictionary (for testing)
#                 data = instance
#             else:
#                 # Otherwise, parse the JSON from the observation
#                 data = json.loads(instance.value_as_string)

#             # Initialize result structure
#             result = {"interest_areas": [], "triggers": []}

#             # Get all interest area IDs from the JSON
#             interest_area_ids = data.get("interest_areas", [])

#             # Fetch all interest area observations at once
...
#                 # Add to result using InterestAreaSerializer
#                 result["interest_areas"].append(InterestAreaSerializer().to_representation(interest_area))

#                 # Get triggers for this interest area
#                 trigger_ids = data.get("triggers", {}).get(str(interest_area.observation_id), [])

#                 # Fetch all trigger observations at once
#                 triggers = Observation.objects.filter(observation_id__in=trigger_ids)

#                 # Process each trigger
#                 for trigger in triggers:
#                     # Add to result using InterestAreaTriggerSerializer
#                     result["triggers"].append(InterestAreaTriggerSerializer().to_representation(trigger))

#             return result

#         except (json.JSONDecodeError, AttributeError) as e:
#             logger.error(f"Error parsing interest area data: {e}")
#             return {"interest_areas": [], "triggers": []}
...
#                 related_triggers = FactRelationship.objects.filter(
#                     domain_concept_1_id=get_concept_by_code("INTEREST_AREA").concept_id,
#                     fact_id_1=interest_area.observation_id,
#                     relationship_concept_id=get_concept_by_code("AOI_TRIGGER").concept_id,
#                 )

#                 # For each related trigger, add its observation as a trigger
#                 for relationship in related_triggers:
#                     trigger_observation = Observation.objects.filter(observation_id=relationship.fact_id_2).first()
#                     if trigger_observation:
#                         validated_data["triggers"].append(
#                             {
#                                 "observation_concept_id": trigger_observation.observation_concept_id,
#                                 "trigger_name": trigger_observation.observation_source_value,
...
class DiaryCreateSerializer(serializers.Serializer):
    ...
    interest_areas = FullInterestAreaSerializer()
...
class DiaryRetrieveSerializer(serializers.Serializer):
    ...
    interest_areas = serializers.SerializerMethodField()  # Changed to SerializerMethodField
    ...
    def get_interest_areas(self, diary):
        data = self._load_json(diary)
        interest_areas_dict = data.get("interest_areas", {})
        interest_areas_dict = interest_areas_dict.get("interest_area_dict", {})

        result = []

        for area_name, area_content in interest_areas_dict.items():

            trigger_dict = area_content.get("trigger_dict", {})

            area = {
                "interest": area_name,
                "triggers": [
                    {"trigger": trigger_name, "value": trigger_value}
                    for trigger_name, trigger_value in trigger_dict.items()
                ],
            }
